...
async function processData() {
    ...
    if (roundData[0].phase === "Test" || roundData[0].phase === "Training") {
      ...
      if (!wentBack) {
        ...
        const twoSecondWindow = relevantData.filter(
          (record) => record.t <= record.coinSpawnTime + 10
        );
        ...
        for (let i = 0; i < twoSecondWindow.length - THRESHOLD; i++) {
          ...
          if (stableCount >= THRESHOLD) {
            ...
            // Store the start time of the stable sequence
            stableSequenceStart = twoSecondWindow[i].t;
            break;
          }
        }
      }
      ...
      processedRecords.push({
        ...
        rt: wentBack
          ...
          ? parseFloat(forwardStartTime.toFixed(3))
          : null,
        stableSequenceStartTime: stableSequenceStart || null,
      });
    }
  }
  ...
  const csvWriter = createCsvWriter({
    ...
    header: [
      ...
      { id: "rt", title: "rt" },
      { id: "stableSequenceStartTime", title: "stableSequenceStartTime" },
