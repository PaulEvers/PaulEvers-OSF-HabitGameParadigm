  ...
  fs.createReadStream(inputFile)
    ...
    .on("end", () => {
      ...
      const processedRows = rows
        .map((row, index) => {
          ...
          if (coinSpawnTime <= time) {
            ...
            for (let i = index + 1; i < rows.length; i++) {
              ...
              if (
                increasingCount >= THRESHOLD ||
                decreasingCount >= THRESHOLD
              ) {
        ...
        .filter((row) => row && row.actionSlip === true); // Filter out null/undefined rows and rows where actionSlip is false
